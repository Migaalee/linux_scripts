#!/bin/bash
# Migla 2022 script for merging files from insilicoseq generated reads. 
...
do
  ...
  sample_strand=$(basename "$sample_file" | cut -f 3 -d "_")
  
  echo " > Adding $sample_file to ${sample_name}_${sample_strand}.${ext}";
  cat $sample_file >> ${output_dir}/${sample_name}_L001_${sample_strand}.${ext};
done
